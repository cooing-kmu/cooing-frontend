...
export default function FreeBoardPost() {
    ...
    async function fetchData() {
      try {
        const response = await axios.get(
          `http://15.165.25.19:8080/board/${boardId}`
        )
    ...
    if (!hasLiked) {
      try {
        const response = await axios.post(
          `http://15.165.25.19:8080/board/${boardId}/likes`
        )
  ...
  const handleScrapClick = async () => {
    if (!hasScrap) {
      try {
        const response = await axios.post(
          `http://15.165.25.19:8080/board/${boardId}/scrap`
        )
        if (response.status === 200) {
          // 스크랩 수 업데이트
          setBoardData((prevData) => ({
  ...
  const handleCommentClick = async () => {
    try {
      await axios.post(`http://15.165.25.19:8080/board/${boardId}/comment`, {
        content: comment,
      })
      setComment('')
      // 댓글 목록을 다시 불러오기
          ...
          <style.ProfileContainer>
            ...
            <style.ProfileTextContainer>
              <style.ProfileTextName>
                {boardData ? boardData.createUserId : '알수없음'}
              </style.ProfileTextName>
